...
import { OeuvreSimular, PopularDrama } from "../../constants/RomanData"
...
const InfosBook = () => {
    ...
    const item = PopularDrama.find((item) => item.name === name);

    const { image , annees , desc , icons , prix , auteur} = item
    ...
    //     axios.post('http://localhost:8800/api/v1/achat_online', { code: uniqueCode }) 
    //     .then((response) => {
    //       if (response.status === 200 || response.status === 201) {
    //         console.log('Code enregistré avec succès');
    //       } else {
    ...
    //   toast.success(`Votre achat est effectué. Code : ${uniqueCode},`, { position: toast.POSITION.TOP_LEFT });
    // }
                ...
                <div className="infos-infos">
                   ...
                   <div className='infos-date'>
                    ...
                    <div className='roman-button'>
                        <button onClick={AchatCode} className='butt1'>Acheter</button>
                        <button onClick={empruntCode} className='butt1'>Emprunter</button>
